import { ModelDefinition, RouteDefinition, ParsedProject } from './typescript-parser';
...
export class DSLValidator {
  ...
  validate(parsed: ParsedProject): ValidationResult {
    ...
    logger.debug(`Validation completed: ${this.errors.length} errors, ${this.warnings.length} warnings`);
    return result;
  }

  private validateModels(models: ModelDefinition[]): void {
    ...
    for (const model of models) {
      ...
      const hasPrimaryKey = model.fields.some(field => 
        field.constraints.some(constraint => constraint.includes('primary'))
      );
      if (!hasPrimaryKey) {
  ...
  private validateModelFields(model: ModelDefinition): void {
  ...
  private validateFieldConstraints(field: any, modelName: string): void {
  ...
  private validateRoutes(routes: RouteDefinition[], models: ModelDefinition[]): void {
  ...
  private validateRouteParameters(route: RouteDefinition): void {
    // Check that all path parameters are defined
    const pathParams: string[] = route.path.match(/:(\w+)/g) || [];
  ...
  private validateRouteTypes(route: RouteDefinition, models: ModelDefinition[]): void {
  ...
  private validateRelationships(models: ModelDefinition[]): void {
    const modelNames = models.map(m => m.name);
  ...
  private addError(message: string, location?: string | undefined): void {
    this.errors.push({ type: 'error', message, location });
  }

  private addWarning(message: string, location?: string | undefined): void {
