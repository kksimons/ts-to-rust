...
async function createProject(config) {
    const projectPath = path_1.default.join(process.cwd(), config.projectName);
    if (!config.dryRun) {
    ...
    logger_1.logger.step('Creating project structure...');
    const structure = [
    ...
    for (const item of structure) {
        ...
        if (config.dryRun) {
            logger_1.logger.debug(`Would create: ${item}`);
            continue;
        }
        if (item.includes('.')) {
            // It's a file
            ...
            // It's a directory
            await fs_extra_1.default.ensureDir(itemPath);
    ...
    if (config.git && !config.dryRun) {
        logger_1.logger.step('Initializing git repository...');
        // TODO: Initialize git repo (spawn git init)
    }
}
async function createProjectFile(filePath, fileName, config) {
    let content = '';
    switch (fileName) {
        case '.env.example':
...
async function createApiFiles(apiPath, _config) {
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(apiPath, 'schema.ts'), schemaContent);
...
`;
    await fs_extra_1.default.writeFile(path_1.default.join(apiPath, 'routes.ts'), routesContent);
}
async function createFrontendFiles(frontendPath, config) {
...
function createEnvExample(config) {
...
function createForgeConfig(config) {
